...
export async function GET() {
  try {
    ...
    const users = await db.user.findMany({
      include: { kursus: true, pensyarah: true },
...
export async function POST(request: NextRequest) {
  try {
    ...
    const kursusId = body.kursusId ? sanitizeString(body.kursusId) : null
    const pensyarahId = body.pensyarahId ? sanitizeString(body.pensyarahId) : null
    const password = typeof body.password === 'string' ? body.password : ''

    if (!name || !email || !role || !password) {
      return Response.json({ error: 'Nama, emel, peranan dan kata laluan diperlukan.' }, { status: 400 })
    }
    if (!Object.values(ROLES).includes(role)) {
      return Response.json({ error: 'Peranan tidak sah.' }, { status: 400 })
    }
    ...
    const created = await db.user.create({
      data: { name, email, passwordHash, role, kursusId, pensyarahId, isDummy: false },
    })
